...
async def test_analyst_returns_findings_with_quotes(mock_llm):
    ...
    mock_llm.chat.return_value = LLMResponse(
        content='{"competitor": "douyin", "dimension": "recommendation", "findings": [{"finding_id": "f001", "claim": "uses deep learning", "quote": "deep learning recommendation engine", "quote_type": "exact", "source_ref": "src001", "chunk_ref": "c001", "reasoning_chain": [{"step": 1, "thought": "official website"}], "confidence": {"score": 0.9, "level": "high"}}], "comparison_matrix": {"dimensions": ["recommendation"], "competitors": {"douyin": {"recommendation": {"status": "Y", "detail": "deep learning"}}}}}',
        model="test",
    )
...
async def test_analyst_filters_findings_without_quote(mock_llm):
    ...
    analyst = Analyst("Analyst", mock_llm)
    mock_llm.chat.return_value = LLMResponse(
        content='{"competitor": "ks", "dimension": "monetization", "findings": [{"finding_id": "f001", "claim": "ks revenue grew", "quote": "", "quote_type": "exact", "source_ref": "", "reasoning_chain": [], "confidence": {"score": 0.5, "level": "low"}}], "comparison_matrix": {"dimensions": [], "competitors": {}}}',
        model="test",
    )
...
async def test_analyst_extracts_reasoning_chain(mock_llm):
    analyst = Analyst("Analyst", mock_llm)
    mock_llm.chat.return_value = LLMResponse(
        content='{"competitor": "bilibili", "dimension": "user_growth", "findings": [{"finding_id": "f001", "claim": "MAU over 200M", "quote": "200 million monthly active users", "quote_type": "exact", "source_ref": "src001", "chunk_ref": "c001", "reasoning_chain": [{"step": 1, "thought": "financial report shows"}], "confidence": {"score": 0.85, "level": "high"}}], "comparison_matrix": {"dimensions": [], "competitors": {}}}',
        model="test",
    )
...
async def test_analyst_reads_min_sources_from_input(mock_llm):
    ...
    analyst = Analyst("Analyst", mock_llm)
    mock_llm.chat.return_value = LLMResponse(
        content='{"competitor": "x", "dimension": "y", "findings": [{"finding_id": "f001", "claim": "test", "quote": "test quote", "quote_type": "exact", "source_ref": "s001", "chunk_ref": "c001", "reasoning_chain": [], "confidence": {"score": 0.9, "level": "high", "uncertainty_factors": []}}], "comparison_matrix": {"dimensions": [], "competitors": {}}}',
        model="test",
    )
